...
def getcommandline():
    ...
    parser.add_argument('--logstarttime',type=int,default=0,help='Minimum T2W to output log.default=0')
    ...
    parser.add_argument('--logstartdepth',type=int,default=0,help='Minimum depth to output log.default=0')
...
def main():
    ...
    if cmdl.tdfilename:
        ...
        ns = (cmdl.logendtime - cmdl.logstarttime) // cmdl.dtout + 1
        # ti = np.linspace(0,cmdl.logendtime,ns).astype(int)
        # ti = np.linspace(0,cmdl.logendtime,ns)
        ti = np.linspace(cmdl.logstarttime,cmdl.logendtime,ns)
        zatt = tout(ti)
        mdx = interpolate.interp1d(devdf.MD.values,devdf.X.values,bounds_error=False,fill_value=np.nan)
        devx_att= mdx(zatt)
        mdy = interpolate.interp1d(devdf.MD.values,devdf.Y.values,bounds_error=False,fill_value=np.nan)
        ...
        logsfnamecsv = 'All' + cmdl.logcolname + f'_{cmdl.logstarttime}_{cmdl.logendtime}.csv'
        ...
        ns = (cmdl.logstartdepth - cmdl.logendtime) // cmdl.dzout + 1
        # zi = np.linspace(0,cmdl.logenddepth,ns).astype(int)
        # zi = np.linspace(0,cmdl.logenddepth,ns)
        zi = np.linspace(cmdl.logstartdepth,cmdl.logenddepth,ns)
        mdx = interpolate.interp1d(devdf.MD.values,devdf.X.values,bounds_error=False,fill_value=np.nan)
        devx_atz= mdx(zi)
        mdy = interpolate.interp1d(devdf.MD.values,devdf.Y.values,bounds_error=False,fill_value=np.nan)
        devy_atz= mdy(zi)
        ...
        logsfnamecsv = 'All' + cmdl.logcolname + f'_{cmdl.logstartdepth}_{cmdl.logenddepth}.csv'
        if os.path.isfile(logsfnamecsv):
